...
from deepseek_client import build_prompt, call_deepseek, call_deepseek_chat
import io
...
if len(radar_choices) >= 2:
	radar_df = df[df["location"].isin(radar_choices)].set_index("location")["safety_score edu_health_rating traffic_score airport_km metro_km".split()]
	fig2 = go.Figure()
...
# --- CHATBOT --- #
st.header("6️⃣ Ask the Real Estate Chatbot")
with st.expander("What can I ask?", expanded=False):
	st.write("Ask about best areas for families, investment, budget fit, commute, or compare localities across Ahmedabad and Gandhinagar.")
...
user_input = st.chat_input("Type your question about localities, budget, safety, commute...")
if user_input:
	st.session_state.chat_messages.append({"role": "user", "content": user_input})
	with st.chat_message("user"):
		st.markdown(user_input)

	# Build system prompt with helpful context
	system_prompt = (
		"You are a helpful, concise real estate assistant for Ahmedabad and Gandhinagar. "
		"Use data-grounded reasoning. If asked about a specific locality name present in the dataset, "
		"focus on safety, traffic, education/health (edu_health_rating), price range, and growth. "
		"When unsure, ask a brief clarifying question. Provide short, actionable answers."
	)

	# Compose message history, optionally augment with a brief context of selected area metrics
	messages = list(st.session_state.chat_messages)
	try:
		assistant_reply = call_deepseek_chat(messages=messages, system=system_prompt)
	except Exception as e:
		assistant_reply = f"Sorry, the AI service is unavailable right now. ({e})"
